...
func _process(delta):
	if can_travel == true and Input.is_action_just_pressed("w"):
		print("travelled")
		get_tree().change_scene("res://Levels/Ruinous Road/FishVillage.tscn")#level the player is going to
		#Checkpoint.door_exit_cords = Checkpoint.starting_level_exit
		Checkpoint.exit_door_cords = Vector2(192, 216) #coordinate player exits at
