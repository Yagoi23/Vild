...
func _process(delta):
	if can_travel == true and Input.is_action_just_pressed("w"):
		print("travelled")
		get_tree().change_scene("res://Levels/Ruinous Road/Starting Level.tscn")#level the player is going to
		#Checkpoint.door_exit_cords = Checkpoint.starting_level_exit
		Checkpoint.exit_door_cords = Vector2(688, 352) #coordinate player exits at




func _on_To_Level_1_area_exited(area):
	if area.is_in_group("Player"): #makes the indicator visible and allows the player to travel
		can_travel = false
		print("no")
		$ArrowDown.visible = false
